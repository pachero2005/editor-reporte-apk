...
def listar_hojas_guardadas():
    ...
    if os.path.exists(carpeta):
        for f in os.listdir(carpeta):
            if f.startswith('Libro_Diario_') and f.endswith('.json'):
                archivos.append(f)
...
def exportar_a_excel(registros):
    ...
    ruta_json_copia = os.path.join(carpeta, f'{nombre_base}.json')

    guardar_datos_json(registros, ruta_json_copia)
    ...
    for r in registros:
        ws.append([r["fecha"], r["detalle"], r["debe"], r["haber"]])

    total_debe = sum(r["debe"] for r in registros)
    total_haber = sum(r["haber"] for r in registros)
...
class LibroDiarioApp(App):
    ...
    def build(self):
        ...
        self.lbl_total_haber = Label(text="Haber: $", font_size='12sp', color=(0.8, 0.7, 0.7, 1), halign='left', valign='middle')
        ...
        self.lbl_diferencia = Label(text="Dif: $", font_size='15sp', bold=True, color=(0.35, 0.85, 0.45, 1), size_hint_x=0.5, halign='right', valign='middle')
    ...
    def mostrar_modal_abrir(self, instance):
        ...
        if not archivos:
            ...
            for nombre_archivo in archivos:
                nombre_visible = nombre_archivo.replace('Libro_Diario_', 'Hoja: ').replace('.json', '')
    ...
    def cargar_para_editar(self, index):
        def _inyeccion(dt):
            ...
            self.txt_fecha['input'].text = str(item["fecha"])
            self.txt_detalle['input'].text = str(item["detalle"])
            self.txt_debe['input'].text = f"{item['debe']:.2f}"
            self.txt_haber['input'].text = f"{item['haber']:.2f}"
    ...
    def actualizar_interfaz(self):
        def _render_tabla(dt):
            ...
            for idx, item in enumerate(self.registros):
                ...
                lbl_det = Label(text=item['detalle'], font_size='15sp', bold=True, color=(1, 1, 1, 1), halign='left', valign='middle')
                lbl_sub = Label(text=f"{item['fecha']}  •  D: ${item['debe']:.2f}  |  H: ${item['haber']:.2f}", font_size='12sp', color=(0.7, 0.75, 0.8, 1), halign='left', valign='middle')
